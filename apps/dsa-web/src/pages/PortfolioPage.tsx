...
const PortfolioPage: React.FC = () => {
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning('券商列表接口返回爲空，已回退爲內置券商列表（華泰/中信/招商）。');
      ...
      setBrokers(FALLBACK_BROKERS);
      setBrokerLoadWarning('券商列表接口不可用，已回退爲內置券商列表（華泰/中信/招商）。');
      if (!FALLBACK_BROKERS.some((item) => item.broker === selectedBroker)) {
        setSelectedBroker(FALLBACK_BROKERS[0].broker);
      }
    }
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || '風險數據獲取失敗，已降級爲僅展示快照數據。');
      }
    } catch (err) {
      setSnapshot(null);
      setRisk(null);
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    if (!name) {
      setAccountCreateError('賬戶名稱不能爲空。');
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        ...
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">手工錄入：公司行爲</h3>
          <form className="space-y-2" onSubmit={handleCorporateSubmit}>
            ...
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>提交企業行爲</button>
          </form>
        </Card>
      </section>
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={eventType} onChange={(e) => setEventType(e.target.value as EventType)}>
                ...
                <option value="corporate">公司行爲</option>
              </select>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: `確認刪除 ${item.effectiveDate} 的公司行爲 ${formatCorporateActionLabel(item.actionType)}（${item.symbol}）嗎？`,
                      })}
                    >
                      刪除
                    </button>
              ...
              {!eventLoading
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      title="暫無流水"
                      description="調整篩選條件或先錄入一筆交易、資金流水或公司行爲。"
