...
const settingsHelpZhCN: SettingsHelpMap = {
  'settings.base.STOCK_LIST': {
    ...
    usage: '多個股票代碼推薦使用英文逗號分隔；從表格或聊天中粘貼時，也會識別中文逗號、頓號、分號、空格和換行，並在保存後規範爲英文逗號。',
    valueNotes: [
      '定時模式每次觸發前會重新讀取當前保存的 STOCK_LIST。',
      '如果命令行臨時傳入 --stocks，隻影響本次手動運行，不會鎖定後續計劃任務。',
      '郵件分組裏的 STOCK_GROUP_N 應寫成 STOCK_LIST 的子集，隻影響郵件收件人，不改變分析範圍。',
    ],
    impact: [
  ...
  'settings.ai_model.GENERATION_BACKEND': {
    ...
    usage: '通常保持“缺省模型配置”。只有在本機已安裝並登錄對應 CLI，且你信任它處理分析內容時，才選擇本地 CLI 生成方式（實驗）。',
    valueNotes: [
      '本地 CLI 生成方式是本機啓動的命令行程序，不等於脫機模型；背後的服務可能處理股票代碼、新聞、持倉上下文、分析請求和報告草稿。',
      'Docker、雲服務器、CI 不天然擁有你本機的登錄狀態；DSA 不讀取 Codex/Claude/OpenCode 登錄憑據文件，但對應 CLI 自己可能使用它的登錄狀態。',
    ],
    impact: ['影響普通分析、大盤覆盤和文本生成入口，不改變問股助手的工具運行規則。'],
    notes: [
      '想恢復缺省行爲，選擇“缺省模型配置”並保存配置。',
      '本地 CLI 生成方式當前仍是實驗能力；如果輸出不穩定或經常失敗，請設回缺省模型配置。',
      '缺省模型配置會繼續使用現有 API Key、模型渠道和備用模型設置。',
    ],
    examples: [],
  },
  'settings.ai_model.GENERATION_FALLBACK_BACKEND': {
    title: '備用生成方式',
    showFieldKey: false,
    summary: '決定本地 CLI 生成失敗後，是直接報錯，還是再嘗試缺省模型配置。',
    usage: '選擇“禁用”表示失敗就報錯；選擇“缺省模型配置”表示再嘗試你已經配置好的普通模型。',
    valueNotes: [
      '如果只是想設置主模型失敗後的備用模型，請使用“備選模型”，不是這個字段。',
      '主生成方式本身就是缺省模型配置時，這個字段不會額外生效。',
    ],
    impact: ['不改變現有備用模型順序，也不會影響渠道編輯器裏的模型配置。'],
    notes: [
      '希望本地 CLI 失敗後立刻暴露錯誤時選擇“禁用”；希望繼續嘗試雲端模型時選擇“缺省模型配置”。',
    ],
    examples: [],
  },
  'settings.ai_model.OPENCODE_CLI_MODEL': {
    ...
    usage: '僅在“分析生成方式”選擇 OpenCode CLI 時生效。留空時 DSA 不傳 --model，使用你本機 OpenCode 的缺省模型配置。',
    valueNotes: [
      '模型是否可用、如何認證由你本機的 OpenCode 配置負責。',
      '配置時該值會作爲單個 argv 參數傳給 OpenCode，不能包含空白或 shell 元字符。',
    ],
  ...
  'settings.ai_model.AGENT_LITELLM_MODEL': {
    title: 'Agent 主模型',
    summary: '爲問股、策略 Agent 等 Agent 鏈路單獨指定模型。',
    usage: '使用 provider/model 格式。留空時繼承普通分析主模型；裸模型名會按兼容邏輯歸一到 openai/<model>。選擇 Codex 本地 Agent 時保留該值，但 Codex 問股不使用它。',
    valueNotes: [
      '適合給 Agent 使用推理能力更強或上下文更長的模型。',
      '該字段隻影響 Agent 鏈路，不會改變普通個股分析的主模型。',
    ],
    impact: ['影響缺省模型問股、策略選擇和相關工具調用；不影響 Codex 本地 Agent。'],
    notes: ['請確認該模型存在於已激活渠道、YAML 路由或 legacy provider key 可達範圍內。'],
  },
  ...
  'settings.ai_model.LLM_PROMPT_CACHE_TELEMETRY_ENABLED': {
    ...
    usage: '缺省打開。關閉後不持久化 provider raw usage JSON、normalized cache fields 和 cache decision diagnostics，基礎 token usage 仍保持兼容。',
    valueNotes: ['該開關不控制 provider implicit cache，也不會改變請求參數。'],
    impact: ['影響 llm_usage 中 provider/cache telemetry 的完整性。'],
    notes: ['如果需要排查 cache hit/miss 或 provider usage shape，保持打開。'],
  },
  'settings.ai_model.LLM_PROMPT_CACHE_HINTS_ENABLED': {
    ...
    usage: '缺省關閉。打開後仍只會對 registry 中 verified 或 smoke-tested 的 provider/route 發送 prompt_cache_key、cache_control 或 user_id 等 hint。',
    valueNotes: ['未知 OpenAI-compatible gateway 缺省 telemetry only，不會自動發送 cache 參數。'],
    impact: ['可能改變主分析路徑的 provider 請求 shape；Agent 路徑當前只記錄 diagnostics，不主動發送 hints。'],
    notes: ['打開前應確認當前 LiteLLM 版本和 provider route 已通過 request-shape 測試。'],
  },
  'settings.ai_model.LLM_PROMPT_CACHE_DIAGNOSTICS_LEVEL': {
    ...
    usage: '可選 off、basic 或 debug。非法值會回退爲 off。',
    valueNotes: [
  ...
  'settings.ai_model.LLM_USAGE_HMAC_SECRET': {
    ...
    notes: ['不要使用登錄 session secret，也不要把真實密鑰提交到版本控制、issue、日誌或截屏裏。'],
  },
  ...
  'settings.data_source.TICKFLOW_API_KEY': {
    ...
    summary: '用於激活 TickFlow A 股日 K、實時行情、股票列表/名稱與大盤覆盤增強數據。',
    ...
    notes: ['不要在 issue、日誌或截屏中暴露真實 Key。'],
  },
  'settings.data_source.TICKFLOW_PRIORITY': {
    title: 'TickFlow 日 K 優先級',
    summary: '控制 TickFlow 在普通 A 股日 K 數據源回退鏈中的位置。',
    usage: '填寫整數；數字越小越早嘗試，缺省 2。未配置 TICKFLOW_API_KEY 時該優先級不會生效；已登記指數固定按 Tencent → AkShare → TickFlow → YFinance 降級，不讀取本配置。',
    valueNotes: ['該設置隻影響普通 A 股日 K 等通用數據源回退鏈，不控制已登記指數或實時行情源順序。'],
    impact: ['影響普通 A 股日 K 獲取的數據源嘗試順序；已登記指數和實時行情均使用各自獨立順序。'],
  ...
  'settings.data_source.TICKFLOW_KLINE_ADJUST': {
    ...
    usage: '可選 none、forward、backward、forward_additive 或 backward_additive。缺省 none。',
    valueNotes: ['none 保持與現有未復權技術指針口徑一致。'],
    impact: ['影響基於 TickFlow 日 K 計算的均線、漲跌幅和其他技術指針口徑。'],
    notes: ['在沒有統一全部數據源復權口徑前，建議保持缺省 none。'],
  },
  'settings.data_source.TICKFLOW_BATCH_DAILY_ENABLED': {
    title: 'TickFlow 批量日 K 預取',
    summary: '控制批量分析時是否先用 TickFlow 批量接口預熱日 K 緩存。',
    usage: '缺省打開。如果當前套餐沒有批量日 K 權限，系統會短期記住失敗狀態並繼續回退。',
  ...
  'settings.data_source.TICKFLOW_BATCH_SIZE': {
    ...
    usage: '填寫正整數，缺省 100。標的數超過該值時系統會拆分多批請求。',
  ...
  'settings.data_source.stock_index_remote': {
    ...
    usage: '缺省打開；如運行環境無法訪問 GitHub raw，可關閉開關。遠程 URL、檢查頻率和超時時間均爲系統內置值。',
    valueNotes: ['系統缺省 48 小時檢查一次更新，避免頻繁訪問 GitHub。', '遠程檢查失敗不會阻斷 WebUI 或分析流程。'],
    impact: ['影響 Web 自動補全和後端股票名稱解析使用的股票簡稱新鮮度。'],
    notes: ['遠程下載失敗時會繼續使用已有緩存或隨應用打包的內置索引。'],
  },
  'settings.base.SCREENING_ENABLED': {
    title: '選股',
    summary: '控制是否激活選股頁；實現參考 AlphaSift。',
    usage: '缺省關閉。設爲 true 後顯示選股入口並激活選股策略。',
    valueNotes: ['選股結果僅用於研究輔助，不構成投資建議。'],
    impact: ['影響 Web 選股入口、策略讀取和選股 API。'],
    notes: ['關閉時不影響原有分析、報告和通知流程。'],
  },
  ...
  'settings.data_source.realtime_quotes': {
    title: '實時行情配置',
    summary: '控制實時行情和盤中技術指針是否激活。',
    usage: '開關字段使用 true/false；行情源順序由 REALTIME_SOURCE_PRIORITY 單獨配置。',
    valueNotes: [
      '關閉實時行情後，分析會更依賴歷史收盤價。',
      '實時技術指針會把盤中價格納入均線和趨勢判斷。',
    ],
    impact: ['影響現價、技術指針、盤中分析和部分報告字段。'],
    notes: ['單一數據源失敗應降級到後續數據源，不應拖垮主流程。'],
  },
  'settings.data_source.FUTU_OPEND_HOST': {
    title: 'Futu OpenD 地址',
    summary: '配置 Futu OpenD 服務地址。留空時不激活 Futu 數據源。',
    usage: '填寫 IPv4 地址或可解析到 IPv4 的主機名。',
    valueNotes: ['OpenD 必須允許 DSA 容器訪問。'],
    impact: ['影響港股 Futu 實時行情、歷史行情和基本面數據訪問。'],
    notes: ['這是服務地址，不是 Futu 帳號密碼。'],
  },
  'settings.data_source.FUTU_OPEND_PORT': {
    title: 'Futu OpenD 端口',
    summary: '配置 Futu OpenD TCP 端口。',
    usage: '填寫 1 到 65535 之間的端口，缺省 11111。',
    valueNotes: ['端口必須與 OpenD 實際監聽端口一致。'],
    impact: ['影響 DSA 與 Futu OpenD 的連接。'],
    notes: ['修改後通常需要重啓 DSA 服務以重新創建連接。'],
  },
  'settings.data_source.FUTU_HK_REALTIME_SOURCE_PRIORITY': {
    title: 'Futu 港股實時數據源優先級',
    summary: '配置港股實時行情的 Futu/Longbridge/AkShare/Yfinance 嘗試順序。',
    usage: '使用英文逗號分隔，可選 futu、longbridge、akshare、yfinance。',
    valueNotes: ['前面的數據源優先嚐試；失敗後自動回退。'],
    impact: ['隻影響港股實時行情，不改變 A 股實時數據源優先級。'],
    notes: ['未配置 Futu OpenD 時會自動跳過 futu。'],
  },
  ...
  'settings.data_source.SEARXNG_BASE_URLS': {
    ...
    usage: '多個實例使用英文逗號分隔；自建實例需要激活 JSON 輸出格式。',
  ...
  'settings.data_source.ENABLE_CHIP_DISTRIBUTION': {
    title: '籌碼分佈分析',
    summary: '控制是否激活籌碼分佈相關分析。',
    usage: '雲部署或數據源不穩定時可設爲 false。',
  ...
  'settings.data_source.pytdx': {
    title: 'Pytdx 通達信服務器',
    summary: '配置通達信行情服務器地址，覆蓋內置缺省服務器。',
  ...
  'settings.notification.FEISHU_WEBHOOK_URL': {
    ...
    usage: '在飛書羣中添加自定義機器人後，拷貝 open-apis/bot/v2/hook 開頭的 Webhook URL 到這裏。',
    valueNotes: [
      '如果機器人打開“簽名校驗”，還需要填寫 FEISHU_WEBHOOK_SECRET。',
      '如果機器人打開“關鍵詞”，還需要填寫 FEISHU_WEBHOOK_KEYWORD，系統會自動補到消息前。',
      'FEISHU_APP_ID / FEISHU_APP_SECRET 用於飛書應用、雲文檔或 Stream Bot，不會直接激活羣 Webhook 推送。',
    ],
    impact: [
  ...
  'settings.notification.FEISHU_STREAM_ENABLED': {
    title: '飛書 Stream 模式',
    summary: '激活飛書應用機器人 / Stream Bot 長連接模式，不是飛書羣 Webhook 推送開關。',
    usage: '只有在已創建飛書應用、完成應用發佈、權限和事件訂閱配置後纔打開；同時需要 FEISHU_APP_ID 和 FEISHU_APP_SECRET。',
    valueNotes: [
      'true 表示允許運行時使用應用機器人 Stream 模式。',
      'false 表示不激活 Stream 模式；羣消息推送仍應使用 FEISHU_WEBHOOK_URL。',
      '只填寫 App ID/Secret 或只打開 Stream，不等於激活羣 Webhook 推送。',
    ],
    impact: [
      '影響飛書應用機器人交互或 Stream Bot 鏈路。',
      '不會改變 FEISHU_WEBHOOK_URL 的羣機器人 Webhook 推送語義。',
    ],
    notes: [
      '保存後通常需要重啓相關 bot/服務進程，已運行的長連接不會自動重建。',
      '失敗只應影響飛書應用機器人鏈路，不應拖垮主分析流程。',
    ],
  },
  'settings.notification.FEISHU_CHAT_ID': {
    title: '飛書 App Bot 推送目標',
    summary: '配置飛書應用機器人主動推送的目標 chat_id（羣聊模式）或 open_id（私聊模式）。',
    usage: '需要同時填寫 FEISHU_APP_ID 和 FEISHU_APP_SECRET。羣聊模式填寫 oc_ 開頭的 chat_id；私聊模式填寫 ou_ 開頭的 open_id 並將 FEISHU_RECEIVE_ID_TYPE 設爲 open_id。',
    valueNotes: [
      '僅憑 FEISHU_APP_ID / FEISHU_APP_SECRET 不會自動激活羣 Webhook 推送。',
  ...
  'settings.notification.FEISHU_RECEIVE_ID_TYPE': {
    title: '飛書接收方 ID 類型',
    summary: '指定 FEISHU_CHAT_ID 的類型：chat_id 表示羣聊，open_id 表示私聊。',
    usage: '羣聊選擇 chat_id；私聊（給指定用戶發 P2P 消息）選擇 open_id。',
    valueNotes: [
  ...
  'settings.notification.DINGTALK_STREAM_ENABLED': {
    title: '釘釘 Stream 模式',
    summary: '激活釘釘應用機器人長連接模式，不是普通釘釘羣機器人 Webhook 開關。',
    usage: '需要先在釘釘開放平臺配置應用機器人，並填寫 DINGTALK_APP_KEY 和 DINGTALK_APP_SECRET。',
    valueNotes: [
      'true 表示允許運行時使用釘釘應用機器人 Stream/長連接模式。',
      'false 表示不激活該長連接模式；自定義 Webhook 中的釘釘羣機器人地址仍走 CUSTOM_WEBHOOK_URLS。',
    ],
    impact: [
      '影響釘釘應用機器人交互或長連接鏈路。',
    ...
    notes: [
      '保存後通常需要重啓相關 bot/服務進程，已運行的長連接不會自動重建。',
      '不要把 Stream 模式和羣機器人 Webhook 混爲一條配置路徑。',
    ],
  },
  'settings.notification.DINGTALK_WEBHOOK_URL': {
    title: '釘釘羣機器人 Webhook',
    summary: '通過普通釘釘羣機器人 Webhook 推送通知，與 App/Stream 模式相互獨立。',
    usage: '在釘釘羣安全設置中添加自定義機器人，拷貝以 oapi.dingtalk.com/robot/send 開頭的 Webhook 地址。',
    valueNotes: [
      'Webhook 包含 access_token，必須按密鑰保護。',
      '若機器人激活了關鍵詞安全模式，通知內容必須包含配置的關鍵詞。',
    ],
    impact: ['影響釘釘羣機器人通知的送達目標。'],
    notes: ['不要填寫釘釘應用 App Key；App/Stream 模式使用獨立配置。'],
  },
  'settings.notification.DINGTALK_SECRET': {
    title: '釘釘羣機器人加簽密鑰',
    summary: '配置釘釘羣機器人安全設置中的 SEC 開頭加簽密鑰。',
    usage: '僅在機器人激活“加簽”安全設置時填寫；未激活加簽時保持爲空。',
    valueNotes: ['該值屬於敏感憑證，Web 設置頁僅顯示遮罩。'],
    impact: ['填寫後，發送器會爲每次 Webhook 請求追加 timestamp 和 sign 參數。'],
    notes: ['關鍵詞和加簽可由釘釘同時要求，需分別滿足。'],
  },
  'settings.notification.webhooks': {
    title: '企業微信 Webhook',
    summary: '配置企業微信羣機器人 Webhook，用於把分析報告推送到指定羣。',
    usage: '在企業微信羣中創建機器人後，拷貝 qyapi.weixin.qq.com/cgi-bin/webhook/send 開頭的 Webhook URL。',
    valueNotes: [
  ...
  'settings.notification.CUSTOM_WEBHOOK_URLS': {
    ...
    valueNotes: [
      ...
      'Docker 部署中保存到 .env 時會自動寫成 $$content_json、$$title_json；運行時仍會還原爲單個 $ 佔位符。',
      '本字段僅影響自定義 Webhook 的 payload 行爲，不會改寫 LLM provider、模型名、Base URL 或遷移優先級。',
    ],
  ...
  'settings.notification.WEBHOOK_VERIFY_SSL': {
    ...
    usage: '缺省保持 true；只有可信內網自簽證書場景才考慮 false。',
    valueNotes: ['關閉校驗會降低中間人攻擊防護。'],
    impact: ['影響所有自定義 Webhook HTTPS 請求的 TLS 校驗行爲。'],
  ...
  'settings.notification.report_output': {
    title: '報告輸出設置',
    summary: '控制通知報告的詳細程度、缺省輸出語言和模板輸出。',
    usage: 'REPORT_TYPE 可選 simple/full/brief，REPORT_LANGUAGE 可選 zh/en/ko；Agent Chat 只有在未顯式傳入 context.report_language 時才繼承這裏的缺省語言。',
    valueNotes: ['報告語言會影響缺省模板、通知文案，以及未單獨指定語言的 Agent Chat 回覆；它不等同於前端接口語言。'],
    impact: ['影響通知正文長度、語言和未顯式指定語言的 Agent Chat 閱讀體驗。'],
  ...
  'settings.system.WEBUI_HOST': {
    ...
    valueNotes: [
      '啓動時顯式傳入的 --host 會優先生效；若未傳 --host，則會使用運行時配置中的 WEBUI_HOST（或其默認值）。',
    ...
    notes: [
      '修改 WEBUI_HOST 後需要重啓當前進程、Docker 容器或服務管理器纔會生效。',
      '直連公網時建議同時激活 ADMIN_AUTH_ENABLED。',
  ...
  'settings.system.WEBUI_PORT': {
    ...
    usage: '本地缺省 8000；如端口衝突可改爲其他 1-65535 範圍內端口。',
    valueNotes: [
      '啓動時顯式傳入的 --port 會優先生效；若未傳 --port，則會使用運行時配置中的 WEBUI_PORT（或其默認值）。',
      'Docker 或雲服務器訪問還取決於宿主機端口映射和安全組。',
      '設置頁保存只會寫入 .env，不會讓當前 WebUI/API 進程重新綁定端口。',
    ],
    impact: ['影響重啓後瀏覽器訪問 WebUI 的 URL 端口。'],
    notes: ['修改 WEBUI_PORT 後需要重啓當前進程、Docker 容器或服務管理器纔會生效。'],
  },
  'settings.system.LOG_DIR': {
    title: '日誌目錄',
    summary: '配置應用日誌輸出目錄。',
    usage: '填寫運行用戶或容器可寫的目錄路徑；本地缺省 ./logs，容器內常見路徑爲 /app/logs。',
    valueNotes: [
      '相對路徑按運行進程的工作目錄解析。',
    ...
    notes: [
      '修改後通常需要重啓進程，已初始化的 logger 不一定會立即切換目錄。',
      'Docker、桌面端和本地原代碼運行的可寫路徑不同，保存前需確認權限。',
    ],
  },
  'settings.system.WEBUI_ENABLED': {
    title: '缺省啓動 WebUI',
    summary: '控制啓動期是否缺省進入 WebUI/API 服務模式。',
    usage: '這是兼容舊啓動入口的啓動期配置；保存後不會讓當前頁面立即啓動或關閉 WebUI。',
    valueNotes: [
      'true 表示後續按缺省入口啓動時傾向進入 WebUI/API 服務模式。',
      'false 表示保持非 WebUI 缺省啓動行爲；顯式 CLI 參數仍可能覆蓋該配置。',
    ],
    impact: [
      '影響 main.py 或相關服務入口下一次啓動時的缺省模式。',
    ],
    notes: [
      '保存後需要重啓相關進程纔會生效。',
      '不要把該開關理解爲當前 Web 設置頁的即時啓停按鈕。',
    ],
  },
  'settings.system.WEBUI_AUTO_BUILD': {
    title: '啓動前自動構建前端',
    summary: '控制後端啓動 WebUI 前是否自動檢查並構建前端靜態產物。',
    usage: '原代碼部署通常保持 true；已預構建鏡像、脫機環境或受限環境可設爲 false。',
    valueNotes: [
      'true 時啓動流程會嘗試準備 apps/dsa-web 靜態產物。',
      'false 時只檢查已有構建產物；如果產物缺失，WebUI 可能不可用或只看到後端警告。',
    ],
    impact: [
      '影響 WebUI 下一次啓動時前端靜態資源是否自動準備。',
    ],
    notes: [
      '保存後不會立即觸發構建，需要重啓相關後端進程。',
      '在 Docker 或發佈包中關閉前，請確認構建產物已經隨鏡像或安裝包提供。',
    ],
  },
  'settings.system.ADMIN_AUTH_ENABLED': {
    title: 'Web 登錄保護',
    summary: '激活 WebUI 管理員密碼保護。',
    usage: '請通過 WebUI 的認證設置入口激活或關閉；忘記密碼可運行 python -m src.auth reset_password。',
    valueNotes: ['直連公網、局域網共享或反向代理部署時建議激活。', '該字段在通用配置頁僅作說明展示，避免繞過認證設置流程。'],
    impact: ['影響 WebUI 登錄、設置頁訪問和管理操作保護。'],
    notes: ['激活前請確認部署環境可以持久化認證數據；手動改 .env 後需要重啓進程或使用認證設置流程刷新狀態。'],
  },
  'settings.system.TRUST_X_FORWARDED_FOR': {
    title: '信任 X-Forwarded-For',
    summary: '在可信反向代理後使用 X-Forwarded-For 識別真實客戶端 IP。',
    usage: '僅單層可信反向代理場景設爲 true；直連公網保持 false。',
    valueNotes: ['多級代理或 CDN 場景下限流 key 可能退化爲邊緣代理 IP。'],
    impact: ['影響登錄限流、審計和真實 IP 識別。'],
    notes: ['不可信代理鏈上打開會允許客戶端僞造來源 IP。'],
  },
  'settings.system.schedule': {
    title: '定時任務',
    summary: '控制是否激活每日定時分析以及啓動時是否立即運行一次。',
    usage: 'SCHEDULE_TIME 使用 HH:MM 24 小時格式；SCHEDULE_TIMES 可配置逗號分隔的多個 HH:MM 時間點；SCHEDULE_ENABLED 控制 runtime scheduler 是否激活；DSA_RUNTIME_SCHEDULER_TIMEOUT_SECONDS 控制單次 Web/API 定時分析的硬超時，缺省 2700 秒、最小 60 秒。',
    valueNotes: [
      '已運行的 schedule 模式會在下一輪調度檢查中讀取新的 SCHEDULE_TIME / SCHEDULE_TIMES 並重建 daily jobs。',
      'WebUI/API/Desktop 長運行進程保存 SCHEDULE_ENABLED、SCHEDULE_TIME 或 SCHEDULE_TIMES 後會按新配置啓停或重建 runtime scheduler。',
      '修改 DSA_RUNTIME_SCHEDULER_TIMEOUT_SECONDS 後，下一次分析會使用新值，無需重建 daily jobs；超時會終止獨立分析進程。',
      '定時任務觸發時會讀取當前保存的 STOCK_LIST。',
    ],
    impact: ['影響 schedule 模式下自動分析頻率、啓動行爲和通知推送時間。'],
    notes: [
      '注意運行環境時區，容器和服務器時區可能與本地不同。',
      'SCHEDULE_RUN_IMMEDIATELY 仍是啓動期行爲；保存後不會立即觸發一次分析。',
    ],
  },
  'settings.system.RUN_IMMEDIATELY': {
    title: '啓動後立即運行',
    summary: '控制非定時模式啓動時是否立即運行一次分析。',
    usage: '需要只啓動服務、不立即分析時設爲 false。',
    valueNotes: ['與 SCHEDULE_RUN_IMMEDIATELY 分別控制非定時和定時模式。'],
    impact: ['影響服務啓動後的首次分析行爲。'],
    notes: [
      '這是非 schedule 模式的啓動期單次運行配置，保存後不會讓當前 WebUI/API 進程立即觸發分析。',
      'CLI 參數和運行模式也會影響最終行爲；修改後需重啓非 schedule 進程後生效。',
    ],
  },
  'settings.system.TRADING_DAY_CHECK_ENABLED': {
    title: '交易日檢查',
    summary: '控制非交易日是否跳過分析。',
    usage: '缺省 true；需要強制運行可設爲 false 或使用 --force-run。',
    valueNotes: ['會結合市場日曆判斷 A 股、港股、美股等市場是否開市。'],
    impact: ['影響定時任務、CLI 和 GitHub Actions 手動運行是否在休市日運行；Web/API 大盤覆盤按鈕會直接提交任務。'],
    notes: ['關閉後休市日可能生成缺少實時行情的報告。'],
  },
  'settings.system.HTTP_PROXY': {
    title: '網絡代理',
    summary: '爲外部 API、模型服務或搜索請求配置代理地址。',
  ...
  'settings.llm_channel.api_surface': {
    title: 'API Surface',
    summary: '選擇該渠道實際調用 Chat Completions 還是 Responses API。',
    usage: '絕大多數兼容服務保持缺省；僅在模型明確要求 Responses API 時選擇 Responses。',
    valueNotes: ['一個渠道內的模型共享同一 API Surface；同一模類型名跨渠道也不能混用兩種 Surface，需要時請使用不同別名。'],
    impact: ['影響連接測試、普通分析、Agent、流式輸出和工具調用的實際端點。'],
    notes: ['Responses 當前僅支持 OpenAI Compatible 協議，模型不能顯式使用 anthropic/、gemini/、xai/ 等其他 LiteLLM provider 前綴；不會在失敗後自動切換端點。'],
  },
  'settings.llm_channel.base_url': {
    title: 'Base URL',
    summary: '該渠道的接口根地址。',
    usage: 'OpenAI-compatible 服務通常填寫以 /v1 結尾的地址；部分官方 SDK 渠道可留空。',
    valueNotes: ['服務商缺省只提供參考值，實際可用性取決於帳號、地區和 provider 當前接口。'],
  ...
  'settings.llm_channel.api_key': {
    ...
    notes: ['不要在截屏、日誌或 issue 中暴露真實 Key。'],
  },
  ...
  'settings.llm_channel.primary_model': {
    title: '主模型',
    summary: '普通分析流程缺省使用的運行時模型。',
    usage: '從已激活渠道的模型列表中選擇；自動模式使用第一個可用模型。',
    valueNotes: ['保存後寫入 LITELLM_MODEL。'],
    impact: ['影響個股分析、大盤覆盤和缺省報告生成。'],
    notes: ['如果模型不在已激活渠道列表中，保存時可能被清理或要求重新選擇。'],
  },
  ...
  'settings.llm_channel.fallback_models': {
    ...
    impact: ['影響 LLM 失敗時的降級成功率和跨 provider 行爲差異。'],
    notes: ['備用模型應先經過連接測試或實際調用驗證。'],
  },
  'settings.llm_channel.vision_model': {
    title: 'Vision 模型',
    summary: '用於截屏識別、圖像輸入或視覺相關能力的模型。',
    usage: '選擇支持圖像輸入的模型；自動模式跟隨缺省 Vision 邏輯。',
    valueNotes: ['保存後寫入 VISION_MODEL。'],
    impact: ['影響圖片/截屏相關提取和視覺分析能力。'],
  ...
  'settings.agent.AGENT_MODE': {
    title: 'Agent 模式',
    summary: '激活 ReAct Agent 進行股票分析，替代普通分析流程。',
    usage: '打開後，系統使用多步推理 Agent 替代單輪 LLM 分析，可調用工具、檢索新聞和運行復雜推理鏈路。',
    valueNotes: [
      '關閉時使用普通單輪 LLM 分析模式。',
  ...
  'settings.agent.AGENT_BACKEND': {
    title: '問股生成方式',
    showFieldKey: false,
    summary: '選擇問股 Chat 使用缺省模型配置，還是調用運行 DSA 設備上的 Codex。',
    usage: '通常保持“自動（推薦）”。自動模式不會激活實驗性的 Codex；只有確認運行 DSA 的設備已安裝並登錄 Codex 後，才選擇 Codex 本地 Agent。',
    valueNotes: [
      '“自動（推薦）”與“缺省模型配置”都繼續使用現有模型和 API 配置。',
      '“Codex 本地 Agent（實驗）”目前只支持單 Agent 問股，不支持 Codex Multi Agent 或 Codex Deep Research。',
      'Codex 本地 Agent 當前支持 macOS、Linux，以及完整運行於 WSL 的 DSA 後端；暫不支持原生 Windows 後端。',
      '本地 Agent 不等於脫機模型；股票問題和工具結果可能由 Codex 自身配置的服務處理。',
    ],
    impact: ['隻影響問股 Chat，不改變普通報告、定時分析、現有 Multi Agent 或 Deep Research。'],
    notes: [
      'DSA 不讀取或保存 Codex 登錄憑據，Codex 進程使用自己的登錄狀態。',
      '設置頁狀態只檢查配置、Codex 命令和所需協議，不會登錄、調用模型或讀取股票數據；顯示“可以嘗試”不代表已驗證可用。',
      '保存後可直接在問股頁提問；第一次問題就是第一次真實運行，若 Codex 登錄或服務不可用，頁面會保留問題並顯示原因。',
      '想恢復原有行爲，選擇“自動（推薦）”並保存。',
    ],
    examples: [],
  },
  'settings.agent.AGENT_GENERATION_BACKEND': {
    title: '問股生成方式',
    showFieldKey: false,
    summary: '決定問股助手用哪種方式生成回覆，並配合工具查找行情、新聞和歷史數據。',
    usage: '通常保持“自動”。系統會選擇當前可用的方式來回答問題並調用數據工具；如果沒有明確要固定方式，無需調整。',
    valueNotes: [
      '如果不確定，選擇“自動”即可。',
      '只有當你明確要固定使用普通模型配置時，才改爲“缺省模型配置”。',
  ...
  'settings.agent.AGENT_MAX_STEPS': {
    title: 'Agent 最大推理步數',
    summary: '控制缺省模型 Agent 的推理步數上限，以及 Codex 單次問股可調用工具的次數上限。',
    usage: '使用缺省模型時，默認值讓每個子 Agent 使用各自缺省步數；調高後統一提升，調低後裁剪缺省步數。使用 Codex 時，這個值限制一次問股最多可以調用多少次數據工具。',
    valueNotes: [
      '步數越高，推理越深入，但耗時和 token 消耗也越大。',
      '部分複雜場景（如多策略編排）可能需要更高步數。',
    ],
    impact: ['影響缺省模型 Agent 的推理深度，或 Codex 單次問股的數據工具調用次數，以及相應耗時和 token 消耗。'],
    notes: ['設爲 0 或極低值可能導致推理不完整。'],
  },
  'settings.agent.AGENT_SKILLS': {
    title: 'Agent 策略列表',
    summary: '指定 Agent 使用的策略技能列表。',
    usage: '使用英文逗號分隔策略名；留空使用缺省策略（bull_trend）；設爲 all 激活全部策略。',
    valueNotes: [
  ...
  'settings.agent.AGENT_SKILL_DIR': {
    ...
    valueNotes: ['缺省 strategies 目錄包含內置策略。'],
    impact: ['影響 Agent 可發現的策略列表。'],
    notes: ['修改目錄後需要確保新目錄中包含有效的策略定義文件。'],
  },
  'settings.agent.AGENT_NL_ROUTING': {
    title: '自然語言路由',
    summary: '允許 bot dispatcher 通過自然語言識別將股票查找路由到 Agent。',
    usage: '打開後，私聊中高置信度的股票相關消息（或羣聊 @機器人）會自動路由到 Agent，無需顯式命令。',
    valueNotes: ['僅影響 bot 接入場景（飛書、Telegram 等），不影響 Web API。'],
    impact: ['影響 bot 交互體驗和 Agent 觸發方式。'],
    notes: ['需要同時激活 Agent 模式和對應 bot 渠道。'],
  },
  'settings.agent.AGENT_ARCH': {
    title: 'Agent 架構',
    summary: '選擇 Agent 運行架構。',
    usage: 'single 使用經典 ReAct 運行器；multi 使用編排器 pipeline，可分配多個專項子 Agent。',
    valueNotes: [
      'single 適合簡單場景，推理鏈路短。',
  ...
  'settings.agent.AGENT_ORCHESTRATOR_TIMEOUT_S': {
    title: 'Agent 超時',
    summary: 'Agent 運行的共享超時預算（秒）。',
    usage: 'single 模式下作爲整體 ReAct 循環超時；multi 模式下作爲協作 pipeline 總超時。設爲 0 禁用超時。',
    valueNotes: [
  ...
  'settings.agent.AGENT_SKILL_CONCURRENCY': {
    title: '策略專家併發數',
    summary: '控制 specialist 模式下最多同時運行多少個策略專家 Agent。',
    usage: '缺省 3，允許範圍 1 到 4。調低可減少瞬時模型調用壓力，調高可縮短多策略批處理等待時間。',
    valueNotes: [
      '該值只限制策略專家 batch 的併發，不改變最終參與綜合的策略選擇。',
      '整體 Agent 超時仍是共享預算；併發數低於策略數時，單個策略會按批量數量分攤剩餘預算。',
    ],
    impact: ['影響 specialist 多策略分析的併發度、耗時和模型調用峯值。'],
    notes: ['單個策略失敗或超時會進入診斷信息，不阻塞其它策略和最終決策。'],
  },
  'settings.agent.AGENT_RISK_OVERRIDE': {
    title: '風險 Agent 否決權',
    summary: '允許風險 Agent 在檢測到關鍵風險信號時否決買入信號。',
    usage: '打開後，full/specialist 模式中的風險 Agent 可將買入建議降級爲觀望或賣出。',
  ...
  'settings.agent.DEEP_RESEARCH': {
    ...
    usage: 'AGENT_DEEP_RESEARCH_BUDGET 設置最大 token 預算；AGENT_DEEP_RESEARCH_TIMEOUT 設置超時秒數。',
    valueNotes: [
  ...
  'settings.agent.AGENT_MEMORY_ENABLED': {
    title: 'Agent 記憶系統',
    summary: '激活記憶與校準系統，跟蹤 Agent 預測準確率並調整置信度。',
    usage: '打開後，系統會記錄每次預測結果，與後續實際走勢對比，用於校準未來分析的置信度。',
  ...
  'settings.agent.AGENT_SKILL_AUTOWEIGHT': {
    title: '策略自動權重',
    summary: '基於真實、可歸因且樣本充足的 Skill Outcome 保守調整策略權重。',
    usage: '打開後，僅當單個 Skill、週期和評估引擎版本獨立達到 30 條 evaluated Outcome 時，系統才使用貝葉斯收縮結果調整綜合信號權重。',
    valueNotes: ['沒有真實 Outcome、樣本不足或統計異常時保持中性權重 1.0。'],
    impact: ['影響多策略綜合時的相對權重，單個性能因子限制在約 0.833 至 1.2。'],
    notes: ['不使用全局回測勝率冒充 Skill 表現；當前平均方向收益不參與權重公式。'],
  },
  ...
  'settings.agent.context_compression': {
    title: '問股上下文壓縮',
    summary: '控制問股可見對話歷史的滾動摘要壓縮，缺省關閉以保持既有行爲。',
    usage: 'AGENT_CONTEXT_COMPRESSION_ENABLED 打開後，僅壓縮同一 session_id 下用戶可見的 user/assistant 文本歷史；profile 控制缺省觸發閾值和保留輪次。',
    valueNotes: [
    ...
    notes: [
      ...
      '當前該 LLM 壓縮只適用於“缺省模型”問股；Codex Agent 始終使用最近 20 條用戶可見對話，不會調用 Agent 主模型生成摘要。保存的壓縮設置不會被清空，切回缺省模型後繼續生效。',
    ],
  },
  'settings.agent.event_monitor': {
    title: '事件監控',
    summary: '在定時模式下激活後臺事件監控，定期輪詢告警規則。',
    usage: 'AGENT_EVENT_MONITOR_ENABLED 打開後臺監控；AGENT_EVENT_MONITOR_INTERVAL_MINUTES 設置輪詢間隔（分鐘）。',
    valueNotes: [
      '僅在定時（schedule）模式下生效。',
      '輪詢間隔過短可能增加 API 調用頻率。',
    ],
  ...
  'settings.agent.EVENT_ALERT_RULES_JSON': {
    ...
    valueNotes: [
      '技術指針、自選股、持倉、大盤紅綠燈等高端規則需通過 Alert API 或 Web 告警中心管理，不在此 JSON 中配置。',
      '規則在定時模式下由事件監控定期評估。',
    ],
    impact: ['影響後臺告警檢測和通知推送。'],
    notes: ['該字段爲 Legacy 配置方式，高端規則請使用告警中心。'],
  },
  // ------------------------------------------------------------------
  ...
  'settings.backtest.BACKTEST_ENABLED': {
    title: '回測開關',
    summary: '激活或關閉歷史分析回測功能。',
    usage: '打開後，系統會定期將歷史分析結果與後續實際走勢對比，評估策略準確率。',
    valueNotes: [
      '回測記錄繼續用於歷史分析評估和現有記憶校準路徑；Skill 自動權重改用獨立的可歸因 Outcome 數據。',
      '關閉回測不影響已有回測記錄，但會停止新回測評估。',
    ],
    impact: ['影響歷史回測評估、記憶校準和回測報告生成；不直接控制 Skill Outcome 權重。'],
    notes: ['AGENT_SKILL_AUTOWEIGHT 不再依賴全局回測勝率。'],
  },
  'settings.backtest.eval_params': {
    title: '回測評估參數',
    summary: '控制回測評估窗口、最小記錄年齡和中性回報帶的參數組。',
    usage: 'BACKTEST_EVAL_WINDOW_DAYS 設置評估窗口（交易日數）；BACKTEST_MIN_AGE_DAYS 僅評估創建時間超過此天數的記錄；BACKTEST_NEUTRAL_BAND_PCT 設置中性回報帶百分比。',
    valueNotes: [
      '評估窗口越長，包含的交易日越多，結果越穩定但延遲越高。',
      'BACKTEST_MIN_AGE_DAYS 用於排除過新的分析記錄，等待足夠後續走勢數據。',
      'BACKTEST_NEUTRAL_BAND_PCT 內的漲跌標記爲中性，不判定爲正確或錯誤。',
    ],
  ...
  'settings.report.REPORT_SUMMARY_ONLY': {
    ...
    usage: '打開後，通知只包含整體摘要信息；關閉後包含每隻股票的詳細分析。',
    valueNotes: ['跟蹤股票較多時打開可減少通知篇幅。'],
  ...
  'settings.report.REPORT_SHOW_LLM_MODEL': {
    ...
    usage: '打開後，通知報告頁腳會顯示模型標識；關閉後隱藏。',
    valueNotes: ['僅影響報告渲染，不改變模型選擇、運行時路由或遷移行爲。'],
    impact: ['影響通知報告中的模型信息可見性。'],
    notes: ['用於排查模型切換問題時可以臨時打開。'],
  },
  'settings.report.REPORT_TEMPLATES_DIR': {
    title: '報告模板目錄',
    summary: 'Jinja2 報告模板的存放目錄。',
    usage: '填寫相對於項目根目錄的路徑；目錄內放置 Jinja2 模板文件。',
    valueNotes: ['需要同時打開 REPORT_RENDERER_ENABLED 纔會使用模板渲染。'],
    impact: ['影響報告渲染使用的模板來源。'],
    notes: ['自定義模板需要符合 Jinja2 語法幷包含必要的變量佔位。'],
  },
  'settings.report.REPORT_RENDERER_ENABLED': {
    title: '報告渲染引擎',
    summary: '激活 Jinja2 模板渲染引擎處理報告輸出。',
    usage: '缺省關閉；打開後報告會通過 Jinja2 模板渲染，支持自定義格式。',
    valueNotes: ['缺省關閉以確保零迴歸風險。'],
    impact: ['影響報告最終輸出格式。'],
    notes: ['打開前請確認 REPORT_TEMPLATES_DIR 中有有效的模板文件。'],
  },
  'settings.report.REPORT_INTEGRITY_ENABLED': {
    title: '報告完整性校驗',
    summary: 'LLM 輸出後校驗必填字段，缺失時重試或使用佔位符。',
    usage: '打開後系統會檢查報告是否包含必要的分析字段；REPORT_INTEGRITY_RETRY 控制重試次數。',
    valueNotes: [
      '校驗失敗的字段會用佔位符填充。',
      '重試會增加 LLM 調用次數和耗時。',
    ],
  ...
  'settings.report.REPORT_HISTORY_COMPARE_N': {
    title: '歷史信號對比',
    summary: '展示每隻股票最近 N 次分析的信號對比。設爲 0 關閉。',
    usage: '打開後，報告中會展示最近 N 次分析信號的對比表格。',
  ...
  'settings.report.SINGLE_STOCK_NOTIFY': {
    ...
    usage: '打開後，每隻股票分析完成後獨立發送通知；關閉後彙總發送。',
    valueNotes: ['打開後通知更及時，但推送頻率更高。'],
    impact: ['影響通知推送時機和頻率。'],
    notes: ['跟蹤大量股票時可能產生較多通知消息。'],
  },
  'settings.report.MERGE_EMAIL_NOTIFICATION': {
    title: '合併郵件通知',
    summary: '將個股分析與大盤覆盤合併爲一封郵件發送。',
    usage: '打開後，個股分析和大盤覆盤會合併在同一封郵件中發送。',
    valueNotes: ['僅在同時激活了個股分析和大盤覆盤時有效。'],
  ...
  'settings.notification.dedup': {
    ...
    usage: 'NOTIFICATION_DEDUP_TTL_SECONDS 設置去重時間窗口，同一去重 key 在窗口內只推送一次；NOTIFICATION_COOLDOWN_SECONDS 設置冷卻時間，同一冷卻 key 在窗口內只推送一次。',
    valueNotes: [
      '兩個機制獨立生效，互不影響。',
      '都設爲 0 時關閉對應去重或冷卻功能。',
    ],
  ...
  'settings.notification.MIN_SEVERITY': {
    ...
    usage: '設爲 warning 時，只有 warning 及以上等級的通知會被推送；留空保留當前行爲。',
    valueNotes: ['等級從低到高依次爲 info、warning、error、critical。'],
    impact: ['影響靜態通知的推送量。'],
  ...
  'settings.notification.DAILY_DIGEST_ENABLED': {
    ...
    usage: '該字段爲 P4 預留功能，當前打開後不會產生任何效果。',
    valueNotes: ['保留用於後續每日摘要聚合推送功能。'],
    impact: ['當前無實際影響。'],
    notes: ['未來版本激活後會聚合當日通知爲一條摘要推送。'],
  },
  // ------------------------------------------------------------------
  // System runtime
  // ------------------------------------------------------------------
  'settings.system.LOG_LEVEL': {
    ...
    impact: ['影響日誌輸出量和詳細程度。'],
    notes: ['修改後部分運行時組件可能需要重啓才能完全生效。'],
  },
  'settings.system.DEBUG': {
    title: '調試模式',
    summary: '打開調試模式，輸出詳細日誌信息。',
    usage: '打開後會輸出更多內部狀態和調試信息。',
    valueNotes: ['生產環境建議關閉以減少日誌量。'],
    impact: ['影響日誌詳細程度和運行時行爲。'],
    notes: ['調試模式可能影響性能，不建議長期打開。'],
  },
  'settings.system.MAX_WORKERS': {
    title: '最大併發線程數',
    summary: '控制同時運行的股票分析線程數量。',
  ...
  'settings.system.ANALYSIS_DELAY': {
    ...
    usage: '設爲 0 無間隔；設爲正值時每完成一隻股票後等待指定秒數再分析下一隻。',
  ...
  'settings.system.SAVE_CONTEXT_SNAPSHOT': {
    ...
    usage: '缺省打開。關閉後，新歷史記錄不會保存 enhanced_context、market_phase_summary、AnalysisContextPack overview 或運行診斷快照等 context_snapshot 內容。',
    valueNotes: [
      '關閉後，新歷史記錄的歷史詳情、completed 任務狀態和 Web 報告頁無法讀取低敏輸入數據塊摘要。',
      '該開關不關閉當次 AnalysisContextPack 構建，也不關閉 LLM Prompt 中的低敏 pack summary。',
      'CLI 的 --no-context-snapshot 與設爲 false 的持久化效果一致。',
    ],
    impact: ['影響歷史透明度、回測/診斷可用的上下文快照信息和 Web 報告頁的數據源摘要。'],
    notes: ['若需要完全關閉 P3-P5 pack 接入，需要回滾相關代碼；當前沒有運行時 pack 總開關。'],
  },
  'settings.system.market_review': {
    title: '大盤分析',
    summary: '控制大盤分析功能的開關、支持的市場子集和配色方案。',
    usage: 'MARKET_REVIEW_ENABLED 打開大盤分析；DAILY_MARKET_CONTEXT_ENABLED 缺省打開，會把當日大盤摘要用於個股分析 Prompt 與保守護欄；MARKET_REVIEW_REGION 支持輸入逗號分隔的市場子集（如 cn,us,jp 或 cn,us,jp,kr），不傳可回退爲 cn；MARKET_REVIEW_COLOR_SCHEME 選擇配色。',
    valueNotes: [
      'cn 覆蓋 A 股，hk 覆蓋港股，us 覆蓋美股，jp 覆蓋日股，kr 覆蓋韓股，both 覆蓋全部（cn,hk,us,jp,kr）。',
      'MARKET_REVIEW_REGION 直接寫入文本框，支持逗號分隔的子集；空值或非法值會回退到 cn。',
      '缺省打開 DAILY_MARKET_CONTEXT_ENABLED；設爲 false 後仍可生成大盤覆盤報告，但個股分析不會讀取大盤摘要或軟化買入/加倉建議。',
      '配色方案影響大盤報告中指數漲跌的顏色顯示：green_up 爲綠漲紅跌，red_up 爲紅漲綠跌。',
    ],
