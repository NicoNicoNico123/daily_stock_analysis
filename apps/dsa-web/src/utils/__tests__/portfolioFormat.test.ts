...
describe('portfolioFormat', () => {
  ...
  it('formats position price fields based on price availability', () => {
    ...
    expect(getPositionPriceLabel(pricedPosition)).toBe('實時價 · longbridge');
  ...
  it('formats broker labels and CSV result variants', () => {
    expect(formatBrokerLabel('huatai')).toBe('huatai（華泰）');
    expect(formatBrokerLabel('custom', ' 自定義 ')).toBe('custom（自定義）');
