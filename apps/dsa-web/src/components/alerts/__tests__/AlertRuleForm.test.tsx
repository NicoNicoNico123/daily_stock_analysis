...
describe('AlertRuleForm', () => {
  ...
  it('submits a price_cross rule payload', async () => {
    ...
    fireEvent.change(screen.getByLabelText('標的代碼'), { target: { value: '600519' } });
    fireEvent.change(screen.getByLabelText('價格閾值'), { target: { value: '1800' } });
    fireEvent.click(screen.getByRole('button', { name: '創建規則' }));
  ...
  it('submits a price_change_percent rule payload', async () => {
    ...
    fireEvent.change(screen.getByLabelText('標的代碼'), { target: { value: 'aapl' } });
    fireEvent.change(screen.getByLabelText('規則類型'), { target: { value: 'price_change_percent' } });
  ...
  it('submits a volume_spike rule payload and supports disabled creation', async () => {
    ...
    fireEvent.change(screen.getByLabelText('標的代碼'), { target: { value: 'msft' } });
    fireEvent.change(screen.getByLabelText('規則類型'), { target: { value: 'volume_spike' } });
    fireEvent.change(screen.getByLabelText('成交量放大倍數'), { target: { value: '2.5' } });
    fireEvent.click(screen.getByLabelText('創建後立即激活'));
    fireEvent.click(screen.getByRole('button', { name: '創建規則' }));
  ...
  it('submits technical indicator rule payloads', async () => {
    ...
    fireEvent.change(screen.getByLabelText('標的代碼'), { target: { value: '600519' } });
    fireEvent.change(screen.getByLabelText('規則類型'), { target: { value: 'macd_cross' } });
    ...
    fireEvent.change(screen.getByLabelText('信號週期'), { target: { value: '5' } });
    fireEvent.click(screen.getByRole('button', { name: '創建規則' }));
  ...
  it('rejects invalid technical indicator boundaries before submit', () => {
    ...
    fireEvent.change(screen.getByLabelText('標的代碼'), { target: { value: '600519' } });
    fireEvent.change(screen.getByLabelText('規則類型'), { target: { value: 'rsi_threshold' } });
  ...
  it('rejects indicator period combinations that exceed fetchable history', () => {
    ...
    fireEvent.change(screen.getByLabelText('信號週期'), { target: { value: '250' } });
    fireEvent.click(screen.getByRole('button', { name: '創建規則' }));

    expect(screen.getByRole('alert')).toHaveTextContent('MACD 週期組合需要 501 根日線，最多支持 365 根');
    expect(onSubmit).not.toHaveBeenCalled();
  });
  ...
  it('rejects empty required technical indicator thresholds before submit', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent('RSI 閾值不能爲空');
    expect(onSubmit).not.toHaveBeenCalled();

    fireEvent.change(screen.getByLabelText('規則類型'), { target: { value: 'cci_threshold' } });
    fireEvent.click(screen.getByRole('button', { name: '創建規則' }));

    expect(screen.getByRole('alert')).toHaveTextContent('CCI 閾值不能爲空');
    expect(onSubmit).not.toHaveBeenCalled();
  });
  ...
  it('rejects invalid stock code format before submit', () => {
    ...
    fireEvent.change(screen.getByLabelText('標的代碼'), { target: { value: 'aapl-2026' } });
    fireEvent.change(screen.getByLabelText('價格閾值'), { target: { value: '200' } });
    fireEvent.click(screen.getByRole('button', { name: '創建規則' }));

    expect(screen.getByRole('alert')).toHaveTextContent('股票代碼格式不正確');
    expect(onSubmit).not.toHaveBeenCalled();
  });
  ...
  it('loads accounts and submits portfolio stop-loss mode', async () => {
    ...
    fireEvent.change(screen.getByLabelText('賬戶'), { target: { value: '9' } });
  ...
  it('renders portfolio alert type options in English UI mode', async () => {
    ...
    expect(screen.queryByText('組合回撤')).not.toBeInTheDocument();
  });
  ...
  it('shows JP/KR options for market region in Chinese UI mode', () => {
    ...
    expect(screen.getByRole('option', { name: '韓股（kr）' })).toBeInTheDocument();
  });
  ...
  it('submits a market light status rule payload', async () => {
    ...
    expect(screen.queryByRole('option', { name: '韓股（kr）' })).not.toBeInTheDocument();
  ...
  it('submits a market light score-drop rule payload', async () => {
    ...
    fireEvent.change(screen.getByLabelText('規則類型'), { target: { value: 'market_light_score_drop' } });
  ...
  it('keeps all account option when account loading fails', async () => {
    ...
    expect(screen.getByLabelText('賬戶')).toHaveValue('all');
  });
  ...
  it('keeps form values when submit reports failure', async () => {
    ...
    expect(screen.getByLabelText('標的代碼')).toHaveValue('aapl');
