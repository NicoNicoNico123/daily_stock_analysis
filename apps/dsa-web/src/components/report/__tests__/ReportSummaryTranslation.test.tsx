...
describe('ReportSummary translation', () => {
  ...
  it('prefers translated report content when the UI language is English', async () => {
    ...
    renderReport(<ReportSummary data={baseReport} isHistory />, 'en');

    // 翻译返回后，正文替换为英文
    await waitFor(() => {
      expect(historyApi.getTranslation).toHaveBeenCalledWith(42, 'en');
    });
  ...
  it('does not fetch a translation when the UI language is Chinese', async () => {
    renderReport(<ReportSummary data={baseReport} isHistory />, 'zh');
