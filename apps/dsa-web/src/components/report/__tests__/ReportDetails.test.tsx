...
describe('ReportDetails', () => {
  ...
  it('keeps copied feedback scoped to the panel that was copied', async () => {
    ...
    const [rawCopyButton, snapshotCopyButton] = screen.getAllByRole('button', { name: '拷貝' });
    ...
    expect(screen.getAllByRole('button', { name: '拷貝' })).toHaveLength(1);
    ...
    expect(screen.getAllByRole('button', { name: '已拷貝' })).toHaveLength(2);

    act(() => {
      vi.advanceTimersByTime(2000);
    });

    expect(screen.getAllByRole('button', { name: '拷貝' })).toHaveLength(2);
  });
