...
const API_SURFACE_OPTIONS: Array<{ value: LLMApiSurface; label: string }> = [
  { value: 'chat_completions', label: 'Chat Completions（默認）' },
...
const ChannelRow: React.FC<ChannelRowProps> = ({
  ...
  const apiSurfaceOptions = API_SURFACE_OPTIONS.some((option) => option.value === channel.apiSurface)
    ? API_SURFACE_OPTIONS
    : [
        { value: channel.apiSurface, label: `無效配置：${channel.apiSurface}` },
        ...API_SURFACE_OPTIONS,
      ];
        ...
        <div className="settings-surface-overlay-soft space-y-4 px-4 py-4">
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            ...
            <Input
              ...
              hint={
                discoveredModels.length > 0
                  ? '如有自定義模型名未出現在列表中，可繼續手動補充，保存格式仍爲逗號分隔。'
...
const LLM_TROUBLESHOOTING_HINTS: Record<string, string> = {
  ...
  model_not_found: '請確認模型名與渠道協議匹配，並先用“獲取模型”覈對該渠道實際可用模型列表。',
  empty_response: '渠道已連通但未返回正文；可嘗試切換兼容模型、關閉額外響應模式後再測試。',
  network_error: '請檢查 Base URL、代理、TLS/證書、中轉網關或本地網絡策略，並可稍後重試。',
  invalid_config: '先補齊協議、Base URL、API Key 和模型配置，再運行一鍵測試。',
  unsupported_protocol: '當前僅對 OpenAI Compatible / DeepSeek 渠道提供自動模型發現，請改爲手動維護模型列表。',
};

const LLM_REASON_HINTS: Record<string, string> = {
  missing_api_key: 'API Key 爲空，或逗號分隔後沒有任何可用 Key；請填入至少一個有效 Key 後再測試。',
  api_key_rejected: '服務商拒絕了當前 API Key；請檢查 Key、組織/項目權限、區域和帳號狀態。',
  rate_limit: '服務商觸發 RPM/TPM 或併發限流；請降低請求頻率或稍後重試。',
  insufficient_balance: '服務商返回餘額、賬單或額度不足；請檢查賬戶餘額和套餐狀態。',
  quota_exceeded: '服務商返回配額已耗盡；請確認帳號套餐、餘量和項目額度。',
  provider_blocked: '請求被服務商或中轉網關攔截；請檢查帳號風控、地域限制、模型權限、代理商網關策略、內容安全策略或請求來源限制。',
  ...
  model_access_denied: '當前帳號無法使用該模型；請確認模型是否已開通、帳號是否可見，或模型是否已被禁用。',
...
function getLlmTroubleshootingHint(
  ...
  if (code === 'format_error') {
    return context === 'discovery' || stage === 'model_discovery'
      ? '該渠道返回的 /models 響應格式不兼容，請改爲手動填寫模型列表。'
      : '返回結構與預期不一致，請確認該渠道兼容 Chat Completions 接口。';
  }
  if (code === 'empty_response' && (context === 'discovery' || stage === 'model_discovery')) {
    return '該渠道的 /models 接口未返回可用模型 ID；請檢查 Base URL 是否指向兼容的模型列表接口，或改爲手動填寫模型列表。';
  }
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: '渠道名稱不能爲空，且只能包含字母、數字或下劃線。' });
      return;
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (mixedPrimary || mixedFallback) {
        setSaveMessage({ type: 'local-error', text: 'Mixed Hermes/non-Hermes route 暫不支持作爲主生成或備選模型，請選擇純 Hermes 或純非 Hermes route。' });
        return;
      }
    ...
    if (managesRuntimeConfig) {
      ...
      if (invalidPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: '當前主模型不在已啓用渠道的模型列表中，請重新選擇。' });
        return;
      }
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between px-1">
              ...
              {channels.length > 0 ? (
                <span className="text-[10px] text-muted-text">{channels.filter((c) => c.enabled).length}/{channels.length} 已啓用</span>
              ) : null}
            </div>
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              ...
              <div className="mb-4">
                ...
                <p className="mt-1 text-[11px] text-secondary-text">
                  控制模型輸出隨機性，0 爲確定性輸出，2 爲最大隨機性，推薦 0.7。
                </p>
              </div>
              ...
              {availableModels.length === 0 ? (
                <div className="rounded-xl border border-dashed settings-border-strong settings-surface-overlay-soft px-3 py-2 text-xs text-muted-text">
                  先添加至少一個已啓用渠道並填寫模型，下面的主模型 / 備選模型 / Vision 選項纔會出現。
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
            ...
            <InlineAlert
              variant="warning"
              message="檢測到已配置高級模型路由 YAML：此處僅管理渠道條目和基礎連接信息。運行時主模型 / 備選模型 / Vision / Temperature 仍由下方通用字段決定；若 YAML 解析成功，則以其中的路由與可用模型聲明爲準，本配置不會覆蓋 YAML 文檔本身。"
