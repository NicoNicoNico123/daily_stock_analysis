...
describe('SettingsPanelErrorBoundary', () => {
  ...
  it('renders a configurable desktop-log diagnostic fallback when a settings panel throws', () => {
    ...
    expect(screen.getByText('通知設置加載失敗')).toBeInTheDocument();
  ...
  it('redacts and truncates sensitive error summary text', () => {
    render(
      <SettingsPanelErrorBoundary title="通知設置" resetKey="notification">
        <ThrowingPanel
  ...
  it('resets after resetKey changes so the panel can render again', async () => {
    const { rerender } = render(
      <SettingsPanelErrorBoundary title="Agent 設置" resetKey="agent:v1">
        <ThrowingPanel />
      </SettingsPanelErrorBoundary>
    );

    expect(screen.getByText('Agent 設置加載失敗')).toBeInTheDocument();

    rerender(
      <SettingsPanelErrorBoundary title="Agent 設置" resetKey="agent:v2">
        <div>Agent 設置已恢復</div>
      </SettingsPanelErrorBoundary>
    );

    await waitFor(() => {
      expect(screen.getByText('Agent 設置已恢復')).toBeInTheDocument();
    });
    expect(screen.queryByText('Agent 設置加載失敗')).not.toBeInTheDocument();
  });
});
