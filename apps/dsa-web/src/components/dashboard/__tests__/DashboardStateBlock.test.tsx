...
describe('DashboardStateBlock', () => {
  it('renders the title as a paragraph by default', () => {
    const { container } = render(<DashboardStateBlock title="開始分析" description="查看提示文案" />);

    const title = screen.getByText('開始分析');
    expect(title.tagName).toBe('P');
    expect(container.querySelector('h3')).toBeNull();
  });

  it('renders the title with the requested heading level', () => {
    render(<DashboardStateBlock title="開始分析" titleAs="h3" description="查看提示文案" />);
  ...
  it('keeps icon, description, action, and loading behaviors intact', () => {
    ...
    expect(screen.getByText('輸入股票代碼進行分析')).toBeInTheDocument();
