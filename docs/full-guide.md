...
| `MACRO_NEWS_ENABLED` | 分析每只股票时是否同时抓取大市宏观热门新闻（美联储利率、政策、大盘走势），并在报告中做「宏观事件 → 行业传导 → 该公司」多点影响分析；设为 `false` 或未配置任何搜索渠道时功能完全关闭 | 默认 `true` |
| `MACRO_NEWS_MAX_RESULTS` | 单次宏观新闻合并后的最大条数（每个市场内置 2 条查询，按 URL 去重合并） | 默认 `5` |
| `MACRO_NEWS_REFRESH_HOURS` | 宏观新闻快照刷新间隔（小时）。宏观新闻按「市场 + 日期」持久化为共享快照（`macro_news_snapshots` 表，全市场/全用户/全进程复用同一份），当日快照在该间隔内直接复用零 API 调用；过期后下一次分析同步重抓；`0` 表示当日只抓一次、永不刷新；抓取失败自动回退当日旧快照，不会阻断分析 | 默认 `6` |
...
| `ENABLE_CHIP_DISTRIBUTION` | 启用筹码分布分析（该接口不稳定，云端部署建议关闭）。GitHub Actions 用户需在 Repository Variables 中设置 `ENABLE_CHIP_DISTRIBUTION=true` 方可启用；workflow 默认关闭。东财/Tushare 筹码接口均失败时，会退化为本地估算（多源日 K + 换手率递衰 CYQ 模型，与东财 CYQCalculator 同源），覆盖 A 股（含 ETF/BSE）、港股与美股（指数跳过），结果 `source=computed` 仅作分析输入、不落库。 | `true` | 可选 |
