...
- [新功能] 报告内容 LLM 翻译（英文界面读取中文报告）：新增 `GET /api/v1/history/{record_id}/translation?lang=en|zh`（zh = 简体中文），复用后端已配置的 LLM 生成链路翻译概览字段/策略点位/Markdown 报告，并按（记录 ID + 目标语言）持久化缓存到 `report_translations` 表（同一记录同一语言只调用一次 LLM）；Web 英文界面在个股报告概览与策略点位区优先渲染翻译结果，翻译未就绪或失败时静默回退原文；中文界面行为不变，记录标题/股票名称/决策动作 taxonomy 不翻译。
