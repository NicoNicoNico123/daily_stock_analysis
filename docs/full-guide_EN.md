...
| `MACRO_NEWS_ENABLED` | Fetch market-wide macro headlines (Fed rate decisions, policy, index moves) alongside per-stock news and run multi-point impact analysis ("macro event → sector transmission → company") in the report. Set to `false` — or run with no search channel configured — to disable the feature entirely | Default `true` |
| `MACRO_NEWS_MAX_RESULTS` | Max merged macro headlines per fetch (each market has 2 built-in queries, deduplicated by URL) | Default `5` |
| `MACRO_NEWS_REFRESH_HOURS` | Macro news snapshot refresh interval in hours. Macro news is persisted as a market-level shared snapshot (`macro_news_snapshots` table, one row per market per UTC date shared by all stocks/users/processes); a same-day snapshot younger than this interval is reused with zero API calls, an expired one is refreshed synchronously on the next analysis, and `0` means fetch once per day and never refresh. When a refresh fails, the previous same-day snapshot is served (`stale` fallback) so analysis is never blocked | Default `6` |
...
| `ENABLE_CHIP_DISTRIBUTION` | Enable chip distribution analysis (this API is unstable, recommended to disable for cloud deployment). GitHub Actions users must set `ENABLE_CHIP_DISTRIBUTION=true` in Repository Variables to enable; disabled by default in workflows. When both the EastMoney and Tushare chip APIs fail, the system falls back to local estimation (multi-source daily K + turnover-decay CYQ model, same lineage as EastMoney's CYQCalculator) covering A-shares (incl. ETF/BSE), Hong Kong and US stocks (indices are skipped); results carry `source=computed` and are analysis input only, never persisted. | `true` | Optional |
